...
def dashboard(request):
    ...
    attending = profile.attending.all().filter(datetime__gte=today).order_by("datetime")
    attended = profile.attending.all().filter(datetime__lt=today).order_by("datetime")
...
def add_review(request, id):
    if request.method == "POST":
        ...
        review = Review.objects.create(text=text, rating=rating, event=event, profile=profile)
        event.reviews.add(review)
        return render (request.POST.get("redirect_url"))
